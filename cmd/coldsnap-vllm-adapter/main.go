...
func main() {
	...
	config := adaptercli.Config{
		Command: "coldsnap-vllm-adapter",
		Engine:  "vllm",
		Operations: []string{
			"capture", "publish", "publish-native", "restore", "sleep", "wake", "status",
		},
		PrepareRestore: true,
		DefaultTimeout: inferenceadapter.DefaultOperationTimeout,
		NewAdapter: func(
			...
			return inferenceadapter.Adapter{
				Engine: "vllm", Remote: remote, StateRoot: stateRoot, Timeout: timeout, Output: output,
			}
		},
	}
